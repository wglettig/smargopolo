...
int main() 
{
    //const string locator = "network:sn:MCS2-00002749"s;
    //const string locator = "network:129.129.129.124"s;
    const string locator = "usb:ix:0"s;
    const unsigned int model = 31001;       // tilt stage
    const double speedLinearAxes = 1e-3;    // 1 mm/s
    const double speedRotaryAxes = 5.;      // 5 degrees/s
    const bool doReferencing = true;


    // open the device
    string options = "locator "s + locator +"\nmodel "s + to_string(model);
