        ...
        if (result == SA_CTL_ERROR_CANCELED) {
            ...
            switch (event.type) {
                ...
                default:
                    ...
                    cout << "MCS2 received unexpected event: " << SA_CTL_GetEventInfo(&event) << endl;
                    break;
                }
        } else {
...
int main() {
    ...
    for (uint32_t i = 0; i < NUMBER_OF_STREAMING_CHANNELS; i++) {
        ...
        exitOnError(result);
        result = SA_CTL_SetProperty_i32(dHandle, i, SA_CTL_PKEY_AMPLIFIER_ENABLED, SA_CTL_ENABLED);
        exitOnError(result);
