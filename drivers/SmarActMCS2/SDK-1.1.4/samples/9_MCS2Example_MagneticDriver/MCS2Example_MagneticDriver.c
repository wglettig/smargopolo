...
void waitWhile(int8_t channel, uint32_t mask) {
    uint32_t state;
    while (1) {
        SA_CTL_Result_t result = SA_CTL_GetProperty_i32(dHandle, channel, SA_CTL_PKEY_CHANNEL_STATE, &state, 0);
        exitOnError(result);
...
int main() {
    ...
    int64_t positions[] = {2000000000,-2000000000,15000000000,-15000000000}; // in pm
    for (uint32_t i = 0; i < 4; i++) {
        printf("MCS2 move channel %d to position: %lld...\n", channel, positions[i]);
    ...
    result = SA_CTL_SetProperty_i32(dHandle, channel, SA_CTL_PKEY_AMPLIFIER_ENABLED, SA_CTL_DISABLED);
    exitOnError(result);
