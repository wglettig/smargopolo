#!/bin/sh
# install script for the SmarActMC SDK for Linux
# (c) 2019-2020 SmarAct GmbH
...
Options:\n\
  ...
  --libraries   Install the SmarActMC libraries only\n\
...
uninstall_libs()
{
  echo "Removing SmarActMC Libraries ($ARCH) from $IPATH"
  rm -f "$IPATH/lib"/libsmaractmc*
...
install_libs()
{
  ...
  echo "Installing SmarActMC Libraries ($ARCH) to $IPATH"
  LIBS_INSTALLED=1

  cp -a -f "$REDIST/arch_$ARCH/lib"/libsmaractmc.so* "$IPATH/lib/"
...
install_sdk_c()
{
  echo "Installing SmarActMC C SDK ($ARCH) to $IPATH"
...
do_uninstall()
{
  echo "Uninstalling SmarActMC software from $IPATH"
  uninstall_libs
  rm -f "$IPATH/include/SmarActMC.h"
}
