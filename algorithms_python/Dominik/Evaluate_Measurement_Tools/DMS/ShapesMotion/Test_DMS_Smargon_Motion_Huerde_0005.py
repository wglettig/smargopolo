...
for i in range(0,5,1):
    print("Move OY to 0.0005")
    response = requests.put(smargopolo_server+'/targetSCS_rel?OY=0.0005')
    time.sleep(2)
    print("Move OX to 0.0005")
    response = requests.put(smargopolo_server+'/targetSCS_rel?OX=0.0005')
    time.sleep(2)
    print("Move OY to -0.0005")
    response = requests.put(smargopolo_server+'/targetSCS_rel?OY=-0.0005')
    time.sleep(2)
    ...
    response = requests.put(smargopolo_server+'/targetSCS_rel?OX=-0.0005')
    time.sleep(2)
    print("Move OZ to +0.0005")
    response = requests.put(smargopolo_server+'/targetSCS_rel?OZ=0.0005')
    time.sleep(2)
...
ax.set_title("3D Plot: Hurdles with Smargon_0005",fontsize=14,fontweight="bold")
ax.legend()
set_axes_equal(ax)
fig.show()
...
with open(current_day + current_time +'Hurdles with Smargon_0005.csv', 'w', newline='') as file:
