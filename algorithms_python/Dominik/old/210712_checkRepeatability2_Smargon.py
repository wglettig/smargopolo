...
def create_CSV_File():
    ...
    with open(day+'checkRepeatability_Smargon.csv', 'w', newline='') as file:
...
#    plt.plot(p_x, '-r', label="Z")
    ...
    with open(day+'checkRepeatability_Smargon.csv', 'a', newline='') as file:
        writer = csv.writer(file)
...
def checkLinearity_GMX_routine():
    ...
    if get_CHI==0 and OmegaRDB == 0:
        ...
        for i in range(0,5,1):
            print("Move OY to +2")
            response = requests.put(smargopolo_server+'/targetSCS_rel?OY=0.01')
            time.sleep(5)
            print("Move OX to +1")
            response = requests.put(smargopolo_server+'/targetSCS_rel?OX=0.01')
            time.sleep(5)
            print("Move OX to +1")
            response = requests.put(smargopolo_server+'/targetSCS_rel?OY=-0.01')
            time.sleep(5)
            ...
            time.sleep(5)
            print("Move OX to +1")
            response = requests.put(smargopolo_server+'/targetSCS_rel?OZ=0.02')
            time.sleep(5)
