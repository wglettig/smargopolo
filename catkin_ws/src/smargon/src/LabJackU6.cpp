// LabJack U6 interface CAP Sensor READOUT
...
// std_msgs/Float32         /LJ_DEG
// Publishers:
// sensor_msgs/JointState   /LJU6_JointState
////////////////////////////////////////////////////////////////////////////////
...
//Globale Variablen
ros::Publisher LJU6_JointState_pub; //LJU6_JointState publisher
sensor_msgs::JointState msgJS; //Raw Message to fill out in prepare msgJS() & publishJointState()

float LJU6_JS[6] = {0.,0.,0.,0.,0.,0.};  //Array to hold values from LJU6 to be published
float OMEGA = 0;

// JointState subscription callback function
// Kann von exernen Aplikationen aufgerufen werden um den OMEGA Wert zu verarbeiten
void LJUE9_JointState_callback(const sensor_msgs::JointState::ConstPtr& msg) {
        //extract omega angle from JointState:
        OMEGA = msg->position[0];
}
// Publish the JointState message
void publishJointState ()  {
    msgJS.header.stamp = ros::Time::now();
    msgJS.position[0] = OMEGA;
    msgJS.position[1] = LJU6_JS[0];
    msgJS.position[2] = LJU6_JS[1];
    msgJS.position[3] = LJU6_JS[2];
    msgJS.position[4] = LJU6_JS[3];
    msgJS.position[5] = LJU6_JS[4];
    LJU6_JointState_pub.publish(msgJS);
}

void prepare_msgJS () { 
    msgJS.name.resize(6);
    msgJS.name[0] = "OMEGA";
    msgJS.name[1] = "LJU6_AIN0";
    msgJS.name[2] = "LJU6_AIN1";
    msgJS.name[3] = "LJU6_AIN2";
    msgJS.name[4] = "LJU6_AIN3";
    msgJS.name[5] = "LJU6_AIN4";
    msgJS.position.resize(6);
}


//LabJack Streaming Functions
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber LJUE9_JointState_sub = n.subscribe("LJUE9_JointState",1000, LJUE9_JointState_callback);
    LJU6_JointState_pub = n.advertise<sensor_msgs::JointState>("LJU6_JointState", 1000);  //LJU6_JointState publisher
    prepare_msgJS();
    ...
    ROS_INFO("Opening LabJackU6...");
    if( (hDevice = openUSBConnection(-1)) == NULL )
        goto done;
    ROS_INFO("Opened.");
   
    //Getting calibration information from U6
    ROS_INFO("Getting Calibration Information...");
    if( getCalibrationInfo(hDevice, &caliInfo) < 0 )
        goto close;
    ROS_INFO("done.");

    ROS_INFO("Setting ConfigIO...");
    if( ConfigIO_example(hDevice) != 0 )
        goto close;
    ROS_INFO("done.");

    //Stopping any previous streams
    ROS_INFO("Stopping any previous streams...");
    StreamStop(hDevice);
    ROS_INFO("done.");

    ROS_INFO("Configure Stream...");
    if( StreamConfig_example(hDevice) != 0 )
        goto close;
    ROS_INFO("done.");

    ROS_INFO("Start Stream...");
...
int StreamData_example(HANDLE hDevice, u6CalibrationInfo *caliInfo)
{
    ...
    numDisplay = 1;
    numReadsPerDisplay = 1; //was 24
    readSizeMultiplier = 1; //original 5 -> decreased reatout speed from 25ms to 5ms Buntschu 1.6.2021
	...
	printf("\nnum Display: %d\n",i);
	    ...
	    LJU6_JS[k] = voltages[scanNumber - 1][k];
	}
	scanNumber=0;
	ros::spinOnce();
	publishJointState();
